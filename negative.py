...
data = []
start = time.clock()
for E in np.linspace(1.2,1,pixels):
    print((1.2-E)/0.2*100,'% done')
    row = []
    for rho in np.linspace(2.9,3,pixels):
        if rho == 1:
            row.append(2)
        else:
            if rho == 3:
...
plt.savefig('negative_fractal_{}.png'.format(pixels))

output = open('negative_greatdata{}.txt'.format(pixels), 'w')
for row in data:
    string = ''
    for column in row:
